<footer class="main-footer" style="height:6.2vh;">
  <div class="float-right d-none d-sm-block">
    <b>Version</b> 1.0
  </div>
  <strong>Copyright &copy; 2020-<?= date('Y') ?> <a href="https://web2rise.com">Web2Rise.com</a>.</strong> All rights reserved.
</footer>

<!-- jQuery -->
<script>
  //   const sidebarOpenFn = () => {
  //     sidebar.classList.remove('sidebar-closed', 'sidebar-collapse');
  //     sidebar.classList.add('sidebar-open');
  //  }
  //  const sidebarCloseFn = () => {
  //     sidebar.classList.add('sidebar-closed', 'sidebar-collapse');
  //     sidebar.classList.remove('sidebar-open');
  //  }
  //   var sidebar = document.querySelector('.sidebar-mini');
  //   var navlink = document.querySelector('.nav-link[data-widget]');
  //   navlink.onclick=sidebarOpenFn;

  //   var contentWrapper = document.querySelector('.content-wrapper');
  //   contentWrapper.onclick=sidebarCloseFn;
</script>
<script src="../assets/password.js"></script>
<script src="https://code.jquery.com/jquery-3.4.1.slim.min.js" integrity="sha384-J6qa4849blE2+poT4WnyKhv5vZF5SrPo0iEjwBvKU7imGFAV0wwj1yYfoRSJoZ+n" crossorigin="anonymous"></script>
<scrip src="https://cdn.jsdelivr.net/npm/popper.js@1.16.0/dist/umd/popper.min.js" integrity="sha384-Q6E9RHvbIyZFJoft+2mJbHaEWldlvI9IOYy5n3zV9zzTtmI3UksdQRVvoxMfooAo" crossorigin="anonymous">
  </script>
  <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/js/bootstrap.min.js" integrity="sha384-wfSDF2E50Y2D1uUdj0O3uMBJnjuUD4Ih7YwaYd1iqfktj0Uod8GCExl3Og8ifwB6" crossorigin="anonymous"></script>
  <script src="https://cdnjs.cloudflare.com/ajax/libs/twitter-bootstrap/4.4.1/js/bootstrap.bundle.min.js" integrity="sha512-c4wThPPCMmu4xsVufJHokogA9X4ka58cy9cEYf5t147wSw0Zo43fwdTy/IC0k1oLxXcUlPvWZMnD8be61swW7g==" crossorigin="anonymous" referrerpolicy="no-referrer"></script>
  <script src="https://cdnjs.cloudflare.com/ajax/libs/jquery/3.6.0/jquery.min.js" integrity="sha512-894YE6QWD5I59HgZOGReFYm4dnWc1Qt5NtvYSaNcOP+u1T9qYdvdihz0PPSiiqn/+/3e7Jo4EaG7TubfWGUrMQ==" crossorigin="anonymous" referrerpolicy="no-referrer"></script>
  <!-- Bootstrap 4 -->
  <!-- <script src="../../plugins/bootstrap/js/bootstrap.bundle.min.js"></script> -->
  <!-- <scrip src="https://unpkg.com/sweetalert/dist/sweetalert.min.js"></script>-->
  <script>
    $('.nav-link[data-widget]').click(function(e) {
      e.stopPropagation();
      $('.sidebar-mini').toggleClass('sidebar-open');
    });
    $('body,html').click(function(e) {
      if(e.target.classList.contains('sidebar-open')){
        console.log("yes");
      }
        $('.sidebar-mini').removeClass('sidebar-open');
    });
  </script>

  </body>

  </html>